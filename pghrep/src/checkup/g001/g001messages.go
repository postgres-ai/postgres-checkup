...
const MSG_HOST_CONCLUSION_HIGH string = "    - server `%s` has %s of RAM, while `shared_buffers` is set to %s, or %d%% of RAM – it is too high, so memory might not be enough, and O[OM killer](https://en.wikipedia.org/wiki/Out_of_memory) might kill Postgres processes since swap is disabled"
...
const MSG_TUNE_SHARED_BUFFERS_RECOMMENDATION string = "Useful links related to `shared_buffers` tuning:  \n" +
	"    - [PostgreSQL documentation. 19.4. Resource Consumption](https://www.postgresql.org/docs/{{MAJOR_VERSION}}/runtime-config-resource.html)  \n" +
